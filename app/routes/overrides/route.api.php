...
$route = function($handler) {
	try {
		...
		$userid = $handler->request[2];
		$album = $handler->request[3];
		...
		if($_FILES['source']['tmp_name']) { // File?
			...
			if(!G\is_image_url($source) && !G\is_url($source)) {
				...
				if(!$api_temp_file or !@is_writable($api_temp_file)) {
					throw new CHV\UploadException("Can't get a tempnam.", 200);
				}
		...
		$_params=[];
		if(!empty($album)) {
			$album = CHV\decodeID($album);
			$_params['album_id'] = $album;
		}
		$uploaded_id = CHV\Image::uploadToWebsite($source, $userid, $_params); 
